"""Native codex decoding shared by more than one Codec."""
...
def decode_base(
    ...
    *,
    ...
    if event != native_event:
        raise RunnerError(
            f"codex {contract_event} codec received a misrouted event {event!r}"
        )
    session_id = raw.get("session_id")
    cwd = raw.get("cwd")
    if not isinstance(session_id, str) or not isinstance(cwd, str):
        raise RunnerError(f"codex {native_event} payload missing 'session_id'/'cwd'")
    return {
...
def decode_tool(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(tool_name, str) or not isinstance(tool_use_id, str):
        raise RunnerError("codex tool payload missing 'tool_name'/'tool_use_id'")
    if "tool_input" not in raw:
        raise RunnerError("codex tool payload missing 'tool_input'")
    tool_input = require_json(raw["tool_input"], "codex tool_input")
    native = require_json_mapping(raw, "codex Native Tool data")
    return {
        "harness": "codex",
        "native": native,
...
def decode_projection(tool_name: str, tool_input: Json) -> dict[str, Any] | None:
...
def require_json(raw: object, what: str) -> Json:
    ...
    if isinstance(raw, list):
        return [require_json(item, f"{what} item") for item in cast(list[object], raw)]
